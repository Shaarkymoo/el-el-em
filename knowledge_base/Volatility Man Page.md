...
usage: {"tool":"<volatility3>","args":[[-h] [-c CONFIG] [--parallelism [{processes,threads,off}]] [-e EXTEND] [-p PLUGIN_DIRS] [-s SYMBOL_DIRS]
...
options:
  ...
  --cache-path CACHE_PATH
                        Change the default path (/home/llmhack/.cache/volatility3) used to store the cache
...
Plugins:
  ...
  PLUGIN
    ...
    linux.modxview.Modxview
                        Centralize lsmod, check_modules and hidden_modules results to efficiently spot modules
                        presence and taints (deprecated).
    ...
    windows.etwpatch.EtwPatch
                        Identifies ETW (Event Tracing for Windows) patching techniques used by malware to evade
                        detection.
    ...
    windows.iat.IAT     Extract Import Address Table to list API (functions) used by a program contained in external
                        libraries
    ...
    windows.malware.processghosting.ProcessGhosting
                        Lists processes whose DeletePending bit is set or whose FILE_OBJECT is set to 0 or Vads that
                        are DeleteOnClose
    windows.malware.psxview.PsXView
                        ...
                        which may help identify processes that are trying to hide themselves.
